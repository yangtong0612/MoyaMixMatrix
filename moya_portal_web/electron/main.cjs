...
#video {
  ...
  width: ${width}px;
  height: ${height}px;
  font-family: ${resolvePupCapsFontFamily(captionStyle.fontFamily)};
}
...
function buildViralTitleAss(overlay, metadata) {
  ...
  return [
    ...
    `Dialogue: 1,0:00:00.00,${titleEnd},Title,,0,0,0,,{\\an5\\pos(${titlePoint.x},${titlePoint.y})}${escapeAssText(wrapAssText(titleText, titleStyle))}`
  ].join('\n');
}
...
function buildViralAss(overlay, metadata) {
  ...
  const titlePrimaryColor = assPrimaryColor(subtitleTemplateOverlay ? overlayTheme.titleColor : '#ffffff');
  const captionPrimaryHex = subtitleTemplateOverlay ? readOverlayCaptionPrimaryColor(overlay) : '#ffffff';
