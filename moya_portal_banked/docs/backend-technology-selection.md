...
| 缓存 | Redis | 可用于后续秒传 hash 缓存、分片上传状态和限流。 |
| 安全 | Spring Security | 网盘业务使用免登录本地用户，保留分享访问控制和旧 JWT 解析兼容能力。 |
...
- 已移除用户注册、登录、重置密码、OAuth 登录和验证码入口，网盘默认进入本地用户业务空间。
...
- 已新增 V2 Flyway 迁移，补齐存储对象、上传分片、分享条目、站内分享等表，并为表和字段添加中文备注。
